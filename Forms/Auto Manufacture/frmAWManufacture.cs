...
{
    public partial class frmAWManufacture : Form
    {
        ...
        public frmAWManufacture(string _id, string _itemCode, string _lotNumber, string _total)
        {
            InitializeComponent();
            id = _id;
            itemCode = _itemCode;
        ...
        private void frmAWManufacture_Load(object sender, EventArgs e)
        {
            setupAWDataTable();
            refreshAWItems();
        }
        ...
        public void getAWLines()
        {
            try
            {
                dataLines = Client.GetAWManufacturePallets(id);
                dataPulled = true;
                if (IsHandleCreated)
                {
        ...
        private void RibtnManuf_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string manufactured = Client.ManufactureAWPallet(id + "|" + lineID + "|" + itemCode + "|" + lotNumber + "|" + qty + "|" + GlobalVars.userName);
        ...
        private void btnManufacture_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (res == DialogResult.Yes)
                {
                    string manufactured = Client.ManufactureAWBatch(id + "|" + itemCode + "|" + lotNumber + "|" + GlobalVars.userName);
                    if (manufactured != string.Empty)
                    {
                        switch (manufactured.Split('*')[0])
                        {
        ...
        private void btnClose_Click(object sender, EventArgs e)
        {
            try
            {
                msg = new frmMsg("Confirm Manual Close", "Are you sure you wish to close this job?" + Environment.NewLine + "Item Code : " + itemCode + Environment.NewLine + "Lot Number : " + lotNumber + Environment.NewLine + "Quantity : " + total, GlobalVars.msgState.Question);
                msg.ShowDialog();
                DialogResult res = msg.DialogResult;
                if (res == DialogResult.Yes)
                {
                    string manufactured = Client.CloseAWBatch(id + "|" + GlobalVars.userName);
                    if (manufactured != string.Empty)
                    {
                        switch (manufactured.Split('*')[0])
                        {
