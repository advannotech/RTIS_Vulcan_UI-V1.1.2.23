...
{
    public partial class ucPOPrinting : UserControl
    {

        DataTable dtPOLines = new DataTable();
        ...
        public void setupDataTable()
        {
            try
            {
                dtPOLines.Columns.Add("gcCode", typeof(string));
                dtPOLines.Columns.Add("gcDesc", typeof(string));
                dtPOLines.Columns.Add("gcDesc2", typeof(string));
                dtPOLines.Columns.Add("gcLotNum", typeof(string));                
                dtPOLines.Columns.Add("gcOrderQty", typeof(string)); //Evo Order Qty
                dtPOLines.Columns.Add("gcRecQty", typeof(string));//Evo Received Qty
                dtPOLines.Columns.Add("gcReceive", typeof(bool)); 
                dtPOLines.Columns.Add("gcPrint", typeof(string));
                dtPOLines.Columns.Add("gcLotLine", typeof(bool));
                dtPOLines.Columns.Add("gcViewable", typeof(bool));

                dtPOLines.Columns.Add("gcRTPrintQty", typeof(string)); //Total Printed
                dtPOLines.Columns.Add("gcValidated", typeof(string));  //Possible to remove?
                dtPOLines.Columns.Add("gcScanned", typeof(string));

                dtPOLines.Columns.Add("gcBack1", typeof(string)); //Qty to receive
                dtPOLines.Columns.Add("gcBack2", typeof(string)); //Running tally of print qty
                dtPOLines.Columns.Add("gcBack3", typeof(string)); //Spare Column
            }
            catch (Exception ex)
            {
        ...
        private void ucPOPrinting_Load(object sender, EventArgs e)
        {
            setupDataTable();
            getSuppliers();
            getPOs();
        ...
        public void refreshPOItems()
        {
            try
            {
                ...
                tmrItems.Start();
                Thread thread = new Thread(getPOLines);
                thread.Start();
            }
            catch (Exception ex)
            {
        ...
        public void getPOLines()
        {
            try
            {
                ...
                {
                    dataLines = Client.GetPOLines(cmbPOs.Text);
                    dataPulled = true;
                }
                else if (cmbSuppliers.Text != string.Empty && cmbPOs.Text == string.Empty)
                {
                    dataLines = Client.GetVendorPOLines(cmbSuppliers.Text);
                    dataPulled = true;
                }
            }
            catch (Exception ex)
                            ...
                            case "1":
                                ...
                                foreach (string item in ItemArray)
                                {
                                    if (item != "")
                                    {
                                        ...
                                        dtPOLines.Rows.Add(item.Split('|'));
                                    }
                                }
                                dgPOItems.DataSource = dtPOLines;
                                dgPOItems.MainView.GridControl.DataSource = dtPOLines;
                            ...
                            if (printed != string.Empty)
                            {
                                switch (printed.Split('*')[0])
                                {
                                    case "1":
                                        ...
                                        int OriLine = 0;
                                        bool lotFound = false;
                                        for (int i = 0; i < dtPOLines.Rows.Count; i++)
                                        {
                                        ...
                                        else
                                        {
                                            ...
                                            DataRow newrow = dtPOLines.NewRow();
                                            ...
                                            dtPOLines.Rows.InsertAt(newrow, OriLine + 1);
                                            #endregion
                                        }
        ...
        private void btnComplete_Click(object sender, EventArgs e)
        {
            try
            {
                DialogResult result = MessageBox.Show("Are you sure to complete this PO, these changes will then be commited to evolution", "RTIS Vulcan", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (result == DialogResult.Yes)
                {
                    ...
                    orderNum = lblPO.Text.Split(':')[1].Replace(" ", string.Empty);
                    ...
                    for (int i = 0; i < dtPOLines.Rows.Count; i++)
                    {
                        ...
                        string code = gvPOItems.GetRowCellValue(i, "gcCode").ToString();
                        string desc = gvPOItems.GetRowCellValue(i, "gcDesc").ToString();
                        string lot = gvPOItems.GetRowCellValue(i, "gcLotNum").ToString();
                        string orderQty = gvPOItems.GetRowCellValue(i, "gcOrderQty").ToString();
                        string recQty = gvPOItems.GetRowCellValue(i, "gcRecQty").ToString();
                        string printQty = gvPOItems.GetRowCellValue(i, "gcRTPrintQty").ToString();
                        string validated = gvPOItems.GetRowCellValue(i, "gcValidated").ToString();
                        string scanned = gvPOItems.GetRowCellValue(i, "gcScanned").ToString();
                        string totalPrinted = gvPOItems.GetRowCellValue(i, "gcBack2").ToString();
                        string userName = GlobalVars.userName;

                        allLines += orderNum + "|" +  code + "|" + lot + "|" + orderQty + "|" + recQty + "|" + printQty + "|" + validated + "|" + scanned + "|" + userName + "~";
                        if (totalPrinted != "0")
                        {
                            string row = code + "|" + desc + "|" + lot + "|" + totalPrinted;
                    ...
                    for (int i = 0; i < 5; i++)
                    {
                        if (dataSend == false)
                        {
                            string posted = Client.postNewPOLines(orderNum + "*" + allLines);
                            switch (posted.Split('*')[0])
                            {
                                case "1":
                                    dataSend = true;
                                ...
                                case "0":
                                    dataSend = true;
                                    ppnlWait.SendToBack();
                                    posted = posted.Remove(0, 2);
                                    msg = new frmMsg("The following server side issue was encountered:", posted, GlobalVars.msgState.Error);
                                    msg.ShowDialog();
                                    break;
                                case "-1":
                                    if (posted.Split('|')[0].Contains("Index was outside the bounds of the array.") == false)
                                    {
                                        dataSend = true;
                                        ppnlWait.SendToBack();
                                        posted = posted.Remove(0, 3);
                                        errMsg = posted.Split('|')[0];
                                        errInfo = posted.Split('|')[1];
                                        ExHandler.showErrorStr(errMsg, errInfo);
                                    }
                                    else
                                    {
                                ...
                                case "-2":
                                    ...
                                    ppnlWait.SendToBack();
                                    posted = posted.Remove(0, 2);
                                    msg = new frmMsg("A connection level error has occured", posted, GlobalVars.msgState.Error);
                                    msg.ShowDialog();
                                    break;
                                default:
                                    dataSend = true;
                                    ...
                                    st = new StackTrace(0, true);
                                    msgStr = "Unexpected error while retreiving purchase order items";
                                    errInfo = "Unexpected error while retreiving purchase order items" + Environment.NewLine + "Data Returned:" + Environment.NewLine + posted;
                                    break;
                            }
                        }
                    }                    
            ...
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    for (int i = 0; i < dtPOLines.Rows.Count; i++)
                    {
                        ...
                        string totalPrinted = gvPOItems.GetRowCellValue(i, "gcBack2").ToString();
                        string userName = GlobalVars.userName;
                        query += "INSERT INTO [tblPOLines] ([vOrderNum]     ,[vItemCode], [vLotNumber]  , [dOrderQty]  , [dRecQty]   , [dPrintQty]  , [bValidated]    , [bScanned], [dtDateEntered], [vUserEntered])" +
                                                  "VALUES  ('" + orderNum + "'  ,'" + code + "', '" + lot + "','" + orderQty.Replace(",", ".") + "', '" + recQty.Replace(",", ".") + "','" + printQty.Replace(",", ".") + "','" + validated + "','" + scanned + "', GETDATE(), '" + userName + "');";
                        allLines += code + "|" + lot + "|" + orderQty + "|" + recQty + "|" + printQty + "~";
                        if (totalPrinted != "0")
                        {
                            string row = code + "|" + desc + "|" + lot + "|" + totalPrinted;
                            dtrep.Rows.Add(row.Split('|'));
                    ...
                    for (int i = 0; i < 5; i++)
                    {
                        if (dataSend == false)
                        {
                            string posted = Client.postNewPOLines(orderNum + "*" + query + "*" + allLines);
                            switch (posted.Split('*')[0])
                            {
                                case "1":
                                    dataSend = true;
                                ...
                                case "0":
                                    ...
                                    break;
                                case "-1":
                                    if (posted.Split('|')[0].Contains("IndexOutOf") == false)
                                    {
                                        dataSend = true;
                                        ppnlWait.SendToBack();
                                        posted = posted.Remove(0, 3);
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            try
            {
                DialogResult result = MessageBox.Show("Are you sure to cancel? this will discard any changes made since the last completion", "RTIS Vulcan", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (result == DialogResult.Yes)
                {

                    string deleted = Client.removeInvalidLabels(lblPO.Text.Split(':')[1].Replace(" ", string.Empty));
                    switch (deleted.Split('*')[0])
                    {
                        ...
                        default:
                            st = new StackTrace(0, true);
                            msgStr = "Unexpected error while removing invalid labels";
                            errInfo = "Unexpected error while removing invalid labels" + Environment.NewLine + "Data Returned:" + Environment.NewLine + deleted;
                            break;
                    }
