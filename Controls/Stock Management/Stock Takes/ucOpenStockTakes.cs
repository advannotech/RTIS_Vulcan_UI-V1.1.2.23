...
{
    public partial class ucOpenStockTakes : UserControl
    {
        DataTable dt = new DataTable();
        DataTable dtEnq = new DataTable();
        bool recordsPulled = false;
        private bool enquiryShowing = false;
        ...
        public void getRTStockTakes()
        {
            try
            {
                string stockTakes = Client.GetRTStockTakes();
        ...
        private void RibtnTickets_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                frmSTTickets stTickets = new frmSTTickets(headerID, itemCode, lotNumber, false);
        ...
        public void getDataLines()
        {
            try
            {
                dataLines = DirectQueries.UI_GetRTStockTakeLines(cmbStockTakes.Text.Split('-')[0].Substring(0, cmbStockTakes.Text.Split('-')[0].Length - 1)); 
                recordsPulled = true;
                if (IsHandleCreated)
                {
        ...
        public void bindDataGrid()
        {
            ...
            enquiryShowing = false;
            btnSwitch.Text = "Enquiry Items";
            pnlLeg.BringToFront();
        }
        private void btnImport_Click(object sender, EventArgs e)
        {
            try
            {
                frmImportST importST = new frmImportST();
                DialogResult result = importST.ShowDialog();
                if (result == DialogResult.OK)
                {
                    string stNum = importST.stNum;
                    getRTStockTakes();
                    cmbStockTakes.Text = stNum;
                    msg = new frmMsg("Success", "Stock take successfully imported", GlobalVars.msgState.Success);
                    msg.ShowDialog();
                }
            }
            catch (Exception ex)
        ...
        private void btnReport_Click(object sender, EventArgs e)
        {
            try
            {
                if (cmbStockTakes.Text != string.Empty)
                {
                    ...
                    if (result == DialogResult.OK)
                    {
                        string paramInfo = Client.GetSTReportInfo(cmbStockTakes.Text.Split('-')[0].Substring(0, cmbStockTakes.Text.Split('-')[0].Length - 1));
                        switch (paramInfo.Split('*')[0])
                        {
                            case "1":
                                ...
                                var query = stReport.sqlDataSource1.Queries[0] as DevExpress.DataAccess.Sql.CustomSqlQuery;
                                query.Sql = string.Format("SELECT * FROM [dbo].[vw_GetItemsBelowSystemCount] WHERE [dbo].[vw_GetItemsBelowSystemCount].[cInvCountNo] = '{0}' ORDER BY [vw_GetItemsBelowSystemCount].[ItemCode]", param);
                                ...
                                query2.Sql = string.Format("SELECT * FROM [dbo].[vw_GetItemsAboveSystemCount] WHERE [dbo].[vw_GetItemsAboveSystemCount].[cInvCountNo] = '{0}' ORDER BY [vw_GetItemsAboveSystemCount].[ItemCode]", param);
                                ...
                                query3.Sql = string.Format("SELECT * FROM [dbo].[vw_GetItemsUncounted] WHERE [dbo].[vw_GetItemsUncounted].[cInvCountNo] = '{0}' ORDER BY [vw_GetItemsUncounted].[ItemCode]", param);
                                ...
                                query4.Sql = string.Format("SELECT * FROM [dbo].[vw_GetItemsEqualSystemCount] WHERE [dbo].[vw_GetItemsEqualSystemCount].[cInvCountNo] = '{0}' ORDER BY [vw_GetItemsEqualSystemCount].[ItemCode]", param);
                                ...
                                query6.Sql = string.Format("SELECT * FROM [dbo].[vw_GetUnlistedItems] WHERE [dbo].[vw_GetUnlistedItems].[cInvCountNo] = '{0}' ORDER BY [vw_GetUnlistedItems].[ItemCode]", param);
        ...
        private void btnComplete_Click(object sender, EventArgs e)
        {
            try
            {
                if (cmbStockTakes.Text != string.Empty)
                {
                    ...
                    if (found == false)
                    {
                        msg = new frmMsg("", "Are you sure you want to complete: " + cmbStockTakes.Text.Split('-')[0].Substring(0, cmbStockTakes.Text.Split('-')[0].Length - 1) + "?", GlobalVars.msgState.Question);
                        msg.ShowDialog();
                        DialogResult res = msg.DialogResult;
                        if (res == DialogResult.Yes)
                        {
                            string archived = Client.ArchiveStockTakeWithTickets(cmbStockTakes.Text.Split('-')[0].Substring(0, cmbStockTakes.Text.Split('-')[0].Length - 1));
                            switch (archived.Split('*')[0])
                            {
                                case "1":
                                    msg = new frmMsg("Success", "Stock Take " + cmbStockTakes.Text.Split('-')[0].Substring(0, cmbStockTakes.Text.Split('-')[0].Length - 1) + " has been archived", GlobalVars.msgState.Success);
                                    msg.ShowDialog();                            
                                    dt.Rows.Clear();
                                    ...
                                    break;
                                case "0":
                                    archived = archived.Remove(0, 2);
                                    msg = new frmMsg("Server error", archived, GlobalVars.msgState.Error);
                                    msg.ShowDialog();
                                    break;
                                case "-1":
                                    archived = archived.Remove(0, 3);
                                    errMsg = archived.Split('|')[0];
                                    errInfo = archived.Split('|')[1];
                                    ExHandler.showErrorStr(errMsg, errInfo);
                                    break;
                                case "-2":
                                    archived = archived.Remove(0, 2);
                                    msg = new frmMsg("A connection level error has occured", archived, GlobalVars.msgState.Error);
                                    msg.ShowDialog();
                                    break;
                                default:
                                    st = new StackTrace(0, true);
                                    msgStr = "Unexpected error while retreiving item tickets";
                                    errInfo = "Unexpected error while retreiving item tickets" + Environment.NewLine + "Data Returned:" + Environment.NewLine + archived;
                                    ExHandler.showErrorST(st, msgStr, errInfo);
                                    break;
                            }
                    ...
                    {
                        msg = new frmMsg("cannot complete stock take", "Cannot complete stock take while there are non-matching quantities ", GlobalVars.msgState.Error);
                        msg.ShowDialog();
                    }
                }
                else
        ...
        public void refreshEnquiry()
        {
            if (cmbStockTakes.Text != "")
            {
                ...
                recordsPulled = false;
                Application.DoEvents();
                Thread thread = new Thread(getEnquiryLines);
                thread.Start();

            }
        }
        public void getEnquiryLines()
        {
            try
            {
                dataLines = DirectQueries.UI_GetRTEnquiryLines(cmbStockTakes.Text.Split('-')[0].Substring(0, cmbStockTakes.Text.Split('-')[0].Length - 1)); 
                recordsPulled = true;
                if (IsHandleCreated)
                {
                    this.Invoke((MethodInvoker)delegate
            ...
            try
            {
                if (recordsPulled == true)
                {
                    if (dataLines != string.Empty)
                    {
                        ...
                        {
                            case "1":
                                dtEnq = DirectQueries.stDTEnquiry;
                                bindDataGrid_Enquiry();
                                ppnlWait.SendToBack();
                                Application.DoEvents();
                                break;
                            case "0":
        ...
        public void bindDataGrid_Enquiry()
        {
            dgEquiry.DataSource = dtEnq;
            dgEquiry.MainView.GridControl.DataSource = dtEnq;
            dgEquiry.MainView.GridControl.EndUpdate();
            dgEquiry.BringToFront();
            enquiryShowing = true;
            btnSwitch.Text = "Normal Items";
            pnlLeg.BringToFront();
        }
